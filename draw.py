...
def generate_spinny(func_in_x, start, end, step): #maybe implement checks for proper functions later
    ...
    for segment_on_curve in range(0, step):        
        x = start + segment_on_curve * dx
        rad = eval(func_in_x) #radius of each circle on curve    

        for circle in range(0, step):
            circ = circle/float(step)
            y = math.sin(2*math.pi * circ) * rad
            z = math.cos(2*math.pi * circ) * rad
...
def generate_stacky(func_in_x, start, end, circumradius, shape, rot, step):
    points = []
    dx = float(end-start)/step
    dr = rot * math.pi  / 180 / step
    
    if shape == "triangle":
        sides = 3
    elif shape == "square":
        sides = 4
    elif shape == "circle": #what is a circle but a regular infinite-gon (or 20-ish gon in graphics-land)?
        sides = step
    int_angle = 180 * (sides - 2) / sides
    
    for segment_on_curve in range(0, step):
        x = start + segment_on_curve * dx
        y = eval(func_in_x)
        z = 0
        direction = segment_on_curve * dr
        for vertex in range(0, sides):
            y += math.sin(direction) * radius 
            z += math.cos(direction) * radius
